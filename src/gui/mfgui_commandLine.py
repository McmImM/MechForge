...
cursor: int  # cursor position in cur_token.
...
all_valid_choices: list[tuple[str, str, PromptKind]]
...
choices: list[tuple[str, str, PromptKind]]
...
active_prompt: tuple[str, str, PromptKind] | None

skip_option = ("Skip", "Skip this option", PromptKind.OPTIONAL)


def reset() -> None:
    global cmd, cur_token, cursor, all_valid_choices, choices, choice, active_prompt

    cmd = []
    cur_token = ""
    cursor = -1
    all_valid_choices = build_prompts([])
...
def text() -> str:
    return " ".join(cmd + ([cur_token] if cur_token else []))
...
def update_choices():
    ...
    choice = 0
    if choices[0][2] is PromptKind.OPTIONAL:
        # If there are only optional choices, select skip_option by default.
        choices.insert(0, skip_option)
...
def pop_token():
    ...
    if cmd and cur_token:
        cur_token = cmd.pop()
        cursor = len(cur_token) - 1
    elif cmd and not cur_token:
        cmd.pop()  # pop the last token from cmd
        if cmd:
            cur_token = cmd.pop()
            cursor = len(cur_token) - 1
    else:
...
def pop_letter():
    global cur_token, cursor
    if cursor >= 0:
        cur_token = cur_token[:cursor]
        cursor -= 1
    else:
        cur_token = cmd.pop() if cmd else ""
        cursor = len(cur_token) - 1
        update_all_valid_choices()
...
def confirm(client: RemoteCore) -> tuple[bool, str]:
    ...
    chosen = choices and choice >= 0

    if chosen and choices[choice] is skip_option:
        # If the user has selected the skip option, we should skip all optional choices.
        return confirm_command(client)

    if cur_token or chosen:
